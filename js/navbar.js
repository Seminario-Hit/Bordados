        ...
        <div class="col-lg-3 col-md-3 col-4 p-0 d-flex justify-content-around align-items-center">
            <button class="btn-icon hide-admin">
                ...
                <a href="../pages/add_new_product.html">
                    <img class="img-icon-addnew" src="../src/img/img-icons/Nuevo_producto.jpg" alt="Imagen para agregar productos">
                </a>
            ...
            <button class="btn-icon hide-user" id="btn-logOut">
                ...
                <a href="#">
                    <img class="img-icon-logout" src="../src/img/img-icons/LogOut.jpg" alt="Imagen de deslogeo">
                </a>
...
let userLogged = window.sessionStorage.getItem("userLogged");
    ...
    // Elementos del DOM
    const loginDropdown = document.querySelector('.img-icon-login').closest('.dropdown');
    ...
    if (tokenJSON != null && userLogged != null) {
        ...
        .then(res => res.json())
        .then(isAdmin => {
            document.querySelectorAll(".hide-user").forEach(el => el.classList.remove("hide-user"));
            loginDropdown.style.display = 'none'; // Ocultar el botón de login

            if (isAdmin) {
                document.querySelectorAll(".hide-admin").forEach(el => el.classList.remove("hide-admin"));
            }
        })
        .catch(error => console.error('Error:', error));
    } else {
        loginDropdown.style.display = 'block'; // Mostrar el botón de login
